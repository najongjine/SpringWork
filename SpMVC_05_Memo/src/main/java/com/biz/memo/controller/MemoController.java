...
/*
 * SessionAtt 로 설정된 변수는(객체)
 * response 하기전에 서버의 기억장소 어딘가에 임시로 보관됨
 * webbrowser와 server간에 한번이라도 연결이 이루어 졌으면
 * session에 등록된 변수는 서버가 중단될때까지 그 값이 그대로 유지된다.
 * web은 특성ㅅ강 클라이언트의 req를 서버가 받아서
 * res를 수행하고마녀 모든 정보가 사라지는 특성이있다.
 * 이런특성관 달리 spring 기반의 web은 일부 데이터들을 메모리에 보관했다가
 * 재사용하는 기법이 있다.
 * */
...
public class MemoController {
	/*
	 * SessionAttribute를 사용하기 위해서는 반드시 해당변수를 생성하는 metod 가 컨트롤러에
	 * 있어야허고 해당 메서드는 ModelAttribute("변수명")가 있어야 한다.
	 * */
	...
	/*
	 * SessionAttribute에서 설정한 변수(객체)에는 @ModelAttribute를 설정해 두어야한다.
	 * */
	 ...
	 * 그 값들을 memoDTO의 필드 변수에 setting을 하고
	 * 만약 없으면 메모리 어딘가에 보관중인 SessionAttribute으로 설정된 
	 * memoDTO변수에서 값을 가져와서 비어있는 필드 변수를 채워서 매개변수에 주입한다.
	 * 
	 * form에서 보이지 않아도 되는 값들은 별도의 코딩을 하지 않아도 자동으로 insert POST로 전송되는
	 * 효과를 낸다.
